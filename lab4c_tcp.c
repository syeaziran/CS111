...
void shut_down() //"outputs (and logs) a final sample with the time and the string SHUTDOWN (instead of a temperature)"
{	
	struct timespec ts;
	struct tm * tm;
	clock_gettime(CLOCK_REALTIME, &ts);
...
int main(int argc, char* argv[]) 
{
	...
	char idbuf[13];
	sprintf(idbuf, "ID=%s\n", id);
	write(sockfd, idbuf, sizeof(idbuf));
	fprintf(file, "ID=%s\n", id);
	...
	while (1) 
	{
		...
		if (pollfds[0].revents & POLLIN)
		{
			...
			int count = read(sockfd, tempBuf, 256);
			if (count < 0)
			{
